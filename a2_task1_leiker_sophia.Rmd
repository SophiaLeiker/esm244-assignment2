...
## 3. Running Model 1 (Variables included: `height`, `length`, `width`, `green_lvs`) on entire dataset
...
**Table 1** Binary Logistic Regression Model Outputs Showing the Variables, Coefficients, Standard Errors
```{r }
...
model_success %>%
  group_by(species) %>% 
  summarize(model_success = mean(accurate)) %>% 
  summarize(m)
#total observations is the n() (since it is after the group function it is already parsed out)
#correctly classified is total
#correct 
...
```{r}
#making a kable table for the output
