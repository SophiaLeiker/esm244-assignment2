...
**Table 1**: NLS Model Results
```{r}
#Making a table with outputs
palmetto_nls_tidy %>% mutate(p.value = case_when(p.value < 0.001 ~ "<0.001")) %>% 
  kable(col.names = c("Term", "Coefficient", "Standard Error", "Statistic", "P Value"), digits = 4) %>% 
...
ggplot() +
  geom_point(data = palmetto, aes(x=sv_length, y = weight, color = sex)) +
  ...
  scale_color_manual(values = c("mediumvioletred", "blue", "limegreen"))
  

```

**Figure 1**: NLS Model Prediction Results for Snout to vent length (mm) vs Weight (g) with colors representing Male (M) and female (F) lizards. The points represent individual data points from the dataset, the blue line represents the model predictions. 
...
- Filtering for variables of interest, transforming to lof weight and sv_length
- Fit NLS Model
...
**Figure 2**: Male Whiptail NLS Model Prediction Results for Snout to vent length (mm) vs Weight (g). The points represent individual data points from the dataset, the purple line represents the model predictions for wight based on NLS run on the full dataset, the green line represents the model predictions based on the NLS model trained with just the Male Whiptails. The RMSE for the NLS model trained on the full model is `r round(palmetto_rmse,3)`. The RMSE for the NLS Model run on male whiptail points is `r round(whiptail_rmse,3)`. For this reason, the model that should be used is the NLS model run on the full dataset. 
